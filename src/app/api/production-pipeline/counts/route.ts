...
let cachedCounts: {
  counts: Awaited<ReturnType<typeof getProductionPipelineCounts>>;
  updatedAt: number;
} | null = null;
...
async function refreshCounts() {
  ...
  const counts = await getProductionPipelineCounts(db);
...
export async function GET() {
  ...
  return NextResponse.json({ counts: cachedCounts!.counts });
}
