...
export async function GET(
  ...
  try {
    ...
    if (localMetadata) {
      return NextResponse.json(localEditedMetadataToEvent(id, localMetadata), {
    ...
    // Append enrichment data only when requested. The event detail page loads
    // its video-first payload from this route and hydrates most enrichments
    // separately, so doing this by default delays the first usable render.
    if (searchParams.get("includeEnrichment") === "true") {
      try {
