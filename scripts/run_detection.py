...
GDINO_TEXT_PROMPT = (
    "person. person riding a bicycle. motorcycle. person on electric scooter. "
...
NON_AMBIGUOUS_CLASSES = {
    ...
    "traffic cone", "skateboard", "scooter",
...
VRU_CLASSES = [
    "person", "person riding a bicycle", "motorcycle", "scooter",
...
def update_heartbeat(conn, run_id: str) -> None:
...
def run_yolo11x(
    ...
    for i, (frame_ms, frame_bgr, frame_idx) in enumerate(frames):
        ...
        results = model.predict(
            frame_bgr, verbose=False, conf=conf_threshold, device=device, imgsz=960
        )[0]
...
def main() -> int:
    ...
    try:
        ...
        # Delete old detections for this run_id (in case of re-run)
        print(f"\n  Deleting old detections for run_id={run_id}...")
        conn.execute(
            "DELETE FROM frame_detections WHERE run_id = ?",
            (run_id,),
        )
        commit_and_sync(conn)

        # Save detections grouped by frame_ms
        print(f"  Saving {len(detections)} detections to DB...")
        frames_dets: dict[int, list[dict]] = defaultdict(list)
        for det in detections:
            frames_dets[det["frame_ms"]].append(det)

        total_saved = 0
        batch_count = 0
        for frame_ms, dets in sorted(frames_dets.items()):
            n = save_detections(conn, video_id, frame_ms, dets, model_name, run_id)
            total_saved += n
            batch_count += 1
            # Heartbeat every 10 frame batches
            if batch_count % 10 == 0:
                update_heartbeat(conn, run_id)

        # Single sync to Turso after all inserts
        if hasattr(conn, "sync"):
            conn.sync()
